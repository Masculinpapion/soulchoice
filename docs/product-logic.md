...
- **NİHAİ KARAR (Mustafa 11.08 gece): Profil "Başvurularım" = ANLIK DURUM PANOSU; kart ömrü = ilan ömrü.** İlan canlıyken (`active`/`selecting`): "Beklemede" (sarı), "Kabul edildi" (yeşil), **reddedilen gri "ЗАВЕРШЕНО/TAMAMLANDI"** — açık "reddedildin" yazmaz ama kullanıcı boş yere umutlanmayıp sıradakine geçer; detay ekranı pasif butonu da aynı etiketi gösterir. **İlan kapanınca kart durumu ne olursa olsun düşer** (ilişki Mesajlar'da). `withdrawn`/`expired` gizli. 30 gün saklama fikri aynı gece İPTAL — cleanup cron 15.07 orijinali (kapalı+match'siz hemen silinir). ✅ 11.08 **REVİZYON (Mustafa 22.08): `accepted` başvuru yalnız ilan `active` iken görünür; süre dolunca (selecting'e geçişte) düşer** — seçim penceresini BEKLEMEZ. Gerekçe: ilan canlıyken yeşil «Принята» sistemin profesyonel işlediğini gösterir; ama süre dolduktan sonra asılı kalması anlamsız (ilişki zaten Mesajlar'da). `pending`/`rejected` için kural değişmedi (active/selecting boyunca görünür). Vaka: Natalia'nın süresi dolmuş (selecting) kartındaki yeşil "Kabul" 2 gün panoda asılı kaldı.
...
- **Kısa tur denetimi 20.08 (moderasyon + sohbet/engel/hesap silme) — düzeltmeler (canlı/pakette):** askıdaki kullanıcı `can_user_apply`'da ACCOUNT_SUSPENDED (paywall'a gidip ödeyemez) ve `match_and_select`'te seçemez/seçilemez; `match_and_select` kabulde `meeting_date` kopyalar (önceden RPC yolunda anket/no-show ölüydü; 2 satır backfill); `confirm_meeting` engelli eşleşmede MATCH_BLOCKED (engellenen, engelleyeni askıya aldıramaz) + uygulamada banner engelli/24 saat kuralına uyar + `meeting_not_yet` metni; `cleanup_closed_invitations` 15.07 'expired' + çift-null match adımları geri; `reports.match_id_snapshot` (eski istemci engel=DELETE olsa da şikâyet mesajları bulunur); sohbet sayfalaması `cleared_at` sınırına uyar. Migration'lar `20260820_suspension_gates_apply_select.sql`, `20260820_chat_match_fixes.sql`. **KARARLAR (Mustafa 20.08): (a) "Разблокировать" butonu KALDIRILDI — engel geri alınamaz (18.08 ile tutarlı), engellenenler listesi salt-okunur; (b) BİR ÇİFT = BİR SOHBET, KALICI: daha önce eşleşmiş (engelsiz) çift yeniden seçilirse yeni sohbet açılmaz, mevcut eşleşme yeni plana bağlanır (invitation_id/kategori/meeting_date yenilenir, teyitler sıfırlanır, gizlilik kalkar) — `match_and_select` + `prevent_matches_tamper` `soulchoice.match_relink_ok` GUC; Mustafa↔Наталья iki sohbeti tek sohbette birleştirildi (96 mesaj) — `20260820_one_chat_per_pair.sql`.** BACKLOG: askı/ban tek trigger (ilan kapat+başvuru+match), karşılıklı no-show (hediye) ops kuyruğu, askı süresi/yeniden kontrol butonu, ops_warn_user bildirimi, banlı girişte `account_banned` metni (Apple sonrası edge), selfie onayında profil cache, ops_remove_photo storage, ops_close_invitation bildirimi, matches UPDATE realtime (engel anlık), tepkiler engelde, tamper kapsamı (meeting_date/hidden), tek fcm_token (cihaz başına değil), hesap silmede CofToken/raw email/name_snapshot izleri (legal-counsel), reports/blocks yeniden kayıtta sıfırlanır.
- **Senaryo denetimi 19.08 (4 alan, kanıtlı) — düzeltmeler:** (a) yenileme günü abone saatlik `downgrade_expired_premium` ile ~14–24 s free'ye düşüyordu (çekim günlük) → "çekim sırada" istisnası (`20260819_downgrade_renewal_window.sql`); (b) `notify_invitation_updated` push 401 (Authorization yok) → düzeltildi + edge 3 dilli şablon + uygulamada push rotası; (c) OTP girişi `setSession` `signedIn` yaymıyordu → girişte push token/hesap dili uygulanmıyordu → accessToken ile çağrı; (d) çıkışta `fcm_token` temizlenmiyordu (başka hesap aynı cihazda eski push'ları alırdı) → `clearPushTokenBeforeSignOut`; (e) telefon ekranı rakam/10 hane doğrulaması; (f) auth dinleyicilerine `onError` (çevrimdışı yenileme 'fatal' sayılmasın); (g) motor Демо guard'ı; (h) `/about` "3 ücretsiz". **Engelleme tek kapı (19.08, canlı): `blocks` INSERT trigger'ı çiftin eşleşmelerini bayraklar (sohbet iki tarafta kapanır, engelleyenden gizlenir) ve bekleyen başvurularını sessizce `withdrawn` yapar — profilden/sohbetten fark etmez (`20260819_block_closes_match_and_apps.sql`).** BACKLOG (karar): ~~feed `limit 30` sunucuda/cinsiyet istemcide~~ → **20.08: karşı cins + yaş filtresi SUNUCUDA (`owner:users!inner` + `or(id.eq.me, and(gender,age))`; Обзор `eq/gte/lte`); 30 kartın 30'u karşı cins; kendi kartım her zaman; istemci filtresi güvenlik ağı**; store/landing "24 saat" + questionnaire "18 yaş" metinleri; tek-seferlik ödeme webhook mutabakatı yok; tek-seferlik premium'da bitiş tarihi görünmüyor; cleanup fonksiyonu 15.07 'expired' adımını kaybetmiş (§4 satır 68 ile çelişir, etkisi yok). BİLGİ: görünür re-login (madde S) kodda mevcut.
