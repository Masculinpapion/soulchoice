...
| C4 | Sosyal push'lar istemci-tetikli | **✅ KAPANDI (26.07, madde X öne çekildi):** mesaj/başvuru/seçildin/eşleşme push'ları DB trigger'larından (pg_net → send-notification); istemci push kodu söküldü; eski istemciler için `push_log` 8sn dedupe; ölü FCM token otomatik temizlenir + loglanır. Migration: `supabase/migrations/20260726_server_push.sql`. Kanıt: istemcisiz SQL mesajı → S24'e 4 sn'de push. |
