...
# insan bekleyen (isaretli VEYA onay-onerili; oneri modu 06.09) selfie 2 saatten eskiyse WARN hatirlatma (Mustafa panelden bakmali).
...
SR_WAIT=$(docker exec supabase-db psql -U postgres -Atc "select count(*) from public.users u where u.selfie_status='pending' and not u.is_test_user and exists (select 1 from public.audit_log a where a.target_id=u.id and a.action in ('selfie_auto_flag','selfie_auto_suggest') and a.ts < now()-interval '2 hours')" 2>/dev/null)
...
elif [ -n "$SR_WAIT" ] && [ "$SR_WAIT" -gt 0 ]; then report "selfie_review" WARN "$SR_WAIT selfie 2 saattir insan onayi bekliyor (supheli/onerili) — Telegram dugmesi veya ops paneli"
