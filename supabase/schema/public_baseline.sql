...
CREATE FUNCTION public.ops_search_users(q text) RETURNS TABLE(id uuid, name text, age integer, city_id text, phone_tail text, created_at timestamp with time zone, verified boolean, banned boolean, warning_count integer, is_test_user boolean, premium boolean, city text)
...
CREATE VIEW public.v_users_all AS
 SELECT users.id,
    ...
    ( SELECT c.name_ru
           FROM public.cities c
          WHERE (c.id = users.city_id)) AS city
   FROM public.users
